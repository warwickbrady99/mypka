...
Pixel's canonical skill recipe (concept → references → prompt → generate → score → deliver) is documented in [[SOP-009-generate-a-styled-image]].
...
- [[SOP-009-generate-a-styled-image]] — Pixel's canonical day-1 skill: concept → references → prompt → generate → score → deliver. Includes the Mack-handoff branch for missing image-gen capability.
- [[SOP-008-build-an-infographic]] — Charta's canonical skill. Pixel reads this when stylizing on top of a Charta-drafted layout.
