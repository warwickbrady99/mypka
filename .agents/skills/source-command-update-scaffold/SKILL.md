---
name: "source-command-update-scaffold"
...
# source-command-update-scaffold

Use this skill when the user asks to run the migrated source command `update-scaffold`.
...
## Portable trigger (not Codex-only)
...
scaffold"). Any LLM driving this scaffold, not only Codex, should run the same
